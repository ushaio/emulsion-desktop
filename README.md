...
- `packages/desktop-plugin-sdk` — 存储插件 TypeScript SDK
- `packages/emulsion-mcp` — 编辑器 MCP 服务器
- `storage_plugins/` — 存储插件运行时（构建时签名打包 Node 运行时）

## 共享包

`@mo-gallery/api-client`、`@mo-gallery/ai-agent`、`@mo-gallery/milkdown`、`@mo-gallery/tiptap-editor` 来自 [mo-gallery-shared](https://github.com/ushaio/mo-gallery-shared)，以 pnpm git 依赖（`#tag&path:`）引用。更新方式见该仓库 README。
