...
export function AdminLayout() {
  ...
  return (
    <div className="desktop-admin-layout flex h-full w-full overflow-hidden">
      <Sidebar onOpenSettings={() => setSettingsOpen(true)} />
      <main className="desktop-content min-w-0 flex-1 overflow-hidden flex flex-col">
        <CachedMenuOutlet />
      </main>
      {settingsOpen && (
        <div className="settings-modal-overlay fixed inset-0 z-40 flex items-center justify-center p-6" role="dialog" aria-modal="true" aria-label="Settings">
          ...
          <div className="desktop-settings-surface lg-sheet relative flex h-[min(900px,calc(100vh-3rem))] w-[min(1180px,calc(100vw-3rem))] min-w-0 flex-col overflow-hidden rounded-lg border shadow-2xl" style={{ backgroundColor: 'var(--background)', borderColor: 'var(--border)' }}>
