// Copied from liquid-glass-react (`.ui/liquid-glass-react/src/shader-utils.ts`, MIT),
// itself adapted from https://github.com/shuding/liquid-glass.
//
// Renders a rounded-rect lens into a canvas and hands back a data URL whose red
// channel carries X displacement and green/blue carry Y, biased to 128. Used by
// the `shader` refraction mode. The loop is O(width * height) with a function
// call per pixel, so callers must bound the texture size -- see
// MAX_SHADER_TEXTURE in LiquidGlass.tsx.
