...
function Modal({ title, description, children, footer, onClose }: { title: string; description: string; children: React.ReactNode; footer: React.ReactNode; onClose: () => void }) { return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-6" onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}><div role="dialog" aria-modal="true" className="desktop-dialog-surface lg-sheet w-full max-w-2xl overflow-hidden rounded-xl border shadow-2xl" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--background)' }}><GlassBackdrop material="regular" /><div className="flex items-start gap-3 border-b px-5 py-4" style={{ borderColor: 'var(--border)' }}><span className="flex size-9 items-center justify-center rounded-lg" style={{ backgroundColor: 'var(--muted)' }}><Server size={16} /></span><div><h3 className="text-sm font-semibold">{title}</h3><p className="mt-1 text-[10px] leading-4" style={{ color: 'var(--muted-foreground)' }}>{description}</p></div></div><div className="max-h-[70vh] overflow-y-auto p-5 custom-scrollbar">{children}</div><div className="flex justify-end gap-2 border-t px-5 py-4" style={{ borderColor: 'var(--border)' }}>{footer}</div></div></div> }
