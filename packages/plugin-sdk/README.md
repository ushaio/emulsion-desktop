...
```ts
import { createStoragePlugin, type StoragePlugin } from '@mo-gallery/plugin-sdk'
...
Use `pnpm --filter @mo-gallery/plugin-sdk test` for the local fake-host
contract tests and `pnpm --filter @mo-gallery/plugin-sdk build` to emit
